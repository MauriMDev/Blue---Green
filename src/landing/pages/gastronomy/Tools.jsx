...
const Tools = () => {
    ...
    try {
      ...
      const formattedTools = response.data.map(tool => {
        ...
        return {
          ...
          image: tool.image || 'https://images.unsplash.com/photo-1599639957043-f3aa5c986398?w=800&q=80',
          type: tool.type || 'cooking',
          material: tool.material || 'N/A',
          origin: tool.origin || 'Tradicional',
          uses: tool.uses || [],
          care: translation.care || 'No disponible',
          history: translation.history || 'No disponible',
        };
      });
      ...
      console.error(err);
      setError(t.gastronomyPage.tools.fetchError || 'Error fetching tools');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Render
  // ==============================
  if (loading) return <div className="text-center py-20">{t.gastronomyPage.tools.loading || 'Loading...'}</div>;
